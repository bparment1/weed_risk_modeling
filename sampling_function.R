############### General function ########## 
## Sampling with various proportion and range for modeling.
## 
...
sampling_training_testing<-function(data_df,nb_sample,step,prop_minmax,obs_id=NULL,seed_number=100,out_suffix="",out_dir="."){
  ...
  #DATE: 06/28/2017                                                                                 
  #PROJECT: Weed infestation, SESYNC based on previous code from Benoit Parmentier from NCEAS     
  #Comments and TODO
  #
  ##################################################################################################
  ...
  if(is.null(obs_id)){
    data_df$obs_id <- 1:nrow(data_df) 
  }else{
    data_df$obs_id <- data_df[[obs_id]]
  }
  ...
  prop_range<-(seq(from=prop_min,to=prop_max,by=step))*100     #range of proportion to run, this is now in percentage
  sn <- nb_sample*length(prop_range)               #Number of samples to run
  ...
  #tab_run_tmp<-cbind(s_nb,prop_tmp)
  #  dates_list[[i]]<-tab_run_tmp
  #}
  
  sampling_dat<- data.frame(id_samp=id_samp,prop_range,nb_sample)
  ...
  list_sampling_training <- vector("list",sn)
  ...
  sampling_obj<-list(sampling_dat,list_sampling_training,list_sampling_testing)
  names(sampling_obj)<- c("sampling_dat","data_training","data_testing")
